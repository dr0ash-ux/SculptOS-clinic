...
await db.exec(readFileSync('supabase/migrations/20260912111939_pharmacy_prescriptions_finance_details.sql','utf8'));
try{await db.exec(readFileSync('supabase/tests/pharmacy_prescriptions.sql','utf8'));console.log('Pharmacy, prescription snapshots, branch isolation and financial metadata checks passed')}catch(e){console.error('PHARMACY ASSERTION',e.message);process.exit(1)}
await db.exec(readFileSync('supabase/migrations/20260912122327_concise_medicines_clinic_letterhead.sql','utf8'));
try{await db.exec(readFileSync('supabase/tests/clinic_letterhead.sql','utf8'))}catch(e){console.error('LETTERHEAD',e.message,e.detail);process.exit(1)}
