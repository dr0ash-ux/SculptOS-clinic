...
export function MiloAssistant({clinicId,clinicName,access,currentPage,onNavigate,onPatient,onBook}:Props) {
  ...
  return <div className="milo-root">
    {open&&<section className="milo-panel" role="dialog" aria-label="Milo clinic assistant" onKeyDown={e=>{if(e.key==='Escape')close()}}>
      <header className="milo-head"><img src={`data:image/webp;base64,${mascot.trim()}`} alt="Milo, a tiny lavender clinic companion"/><div><strong>Milo <small>EARLY ACCESS</small></strong><span>{clinicName}</span></div><button type="button" onClick={close} aria-label="Close Milo"><X size={19}/></button></header>
    ...
    <button type="button" className="milo-launcher" ref={launcher} aria-expanded={open} aria-label={`Milo assistant${due?`, ${due} reminders due`:''}`} onClick={()=>setOpen(!open)}><img src={`data:image/webp;base64,${mascot.trim()}`} alt=""/><span>{open?'Close Milo':'Ask Milo'}</span>{due>0&&<b aria-live="polite">{due}</b>}</button>
  </div>
}
