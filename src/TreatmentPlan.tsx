...
export function TreatmentPlan({ patient, workspace, clinicianName, onNotice, onBack, onAppointments }: {
  ...
  const canOverride = usePermission('patient_price.override')
  const canEdit = usePermission('treatment_plans.write')
  const canDiscount = usePermission('treatment_discount.apply')
  const canRemove = usePermission('treatment_plans.delete')
  ...
  return <section className="treatment-plan-page">
    ...
    <form className="clinical-section treatment-plan plan-editor" onSubmit={submit}>
      ...
      {loading ? <p role="status">Loading treatment plan…</p> : <>
        {!canEdit && <div className="controls-hint">Read-only access · Ask your administrator to enable treatment editing. <button type="button" className="ghost" disabled={loading||saving||!!error||!included.length} onClick={()=>{document.body.classList.add('treatment-plan-print-ready');window.print()}}>Print saved plan</button></div>}<fieldset disabled={saving || !canEdit} className="plan-fields">
