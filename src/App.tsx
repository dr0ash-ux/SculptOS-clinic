...
function Inventory(){const [notice,setNotice]=useState('');const stock=[['Anesthetic cartridges','Anaesthetic','82','₹42'],['Surgical gloves · M','PPE','240','₹9'],['Implant components','Implant','18','₹4,800'],['Suture · 3-0','Surgical','64','₹38']];return <section><div className="hero"><div><span className="eyebrow">ADMIN · INVENTORY</span><h1>Inventory & supplies</h1><p>Current stock, usage, wastage and procurement in one clinic view.</p></div><div className="hero-actions"><button className="secondary compact" onClick={()=>setNotice('Record usage selected — this creates a stock movement, not a new expense.')}>Record usage</button><button className="primary compact" onClick={()=>setNotice('Stock-in selected — a costed purchase creates one linked finance expense.')}><Plus size={16}/> Stock-in</button></div></div>{notice&&<div className="booking-toast">{notice}<button onClick={()=>setNotice('')}>×</button></div>}<div className="stats"><Stat label="Current stock" value="248 items" delta="14 low stock"/><Stat label="Inventory value" value="₹8.6L" delta="At unit cost"/><Stat label="Usage this month" value="₹42.3K" delta="vs ₹38.1K"/><Stat label="Wastage this month" value="₹3.2K" delta="Reported separately"/></div><div className="grid2"><div className="panel"><div className="panel-head"><div><b>Month comparison</b><span>Current month vs last month</span></div></div><div className="pipeline"><Pipeline n="1,284 / 1,143" l="Units used"/><Pipeline n="46 / 31" l="Units wasted"/><Pipeline n="₹71K / ₹64K" l="Purchase spend"/></div></div><div className="panel"><div className="panel-head"><div><b>Low-stock alerts</b><span>Items at or below threshold</span></div></div><div className="timeline"><div className="appt-row"><span>18</span><div><b>Implant components</b><small>Threshold: 20 pieces</small></div><em>Reorder</em></div><div className="appt-row"><span>24</span><div><b>Gutta-percha</b><small>Threshold: 30 points</small></div><em>Reorder</em></div></div></div></div><div className="panel table-panel"><div className="table-head"><b>Stock & movement history</b><button onClick={()=>setNotice('Choose an item to view every purchase, usage, wastage and adjustment.')}>View history</button></div><table><thead><tr><th>Item</th><th>Category</th><th>Stock</th><th>Unit cost</th><th>Status</th></tr></thead><tbody>{stock.map(([name,category,amount,cost])=><tr key={name}><td><b>{name}</b></td><td>{category}</td><td>{amount}</td><td>{cost}</td><td><em className="pill">{name==='Implant components'?'Low stock':'Healthy'}</em></td></tr>)}</tbody></table></div></section>}

function Finance(){const [range,setRange]=useState('Current month');const [notice,setNotice]=useState('');const [prices,setPrices]=useState([{name:'Root canal treatment',price:'6500'},{name:'Implant placement',price:'32000'},{name:'Consultation',price:'800'}]);return <section><div className="hero"><div><span className="eyebrow">ADMIN · FINANCE</span><h1>Financial overview</h1><p>Clinic financials, payment collection and treatment-price control.</p></div><div className="hero-actions"><select value={range} onChange={e=>setRange(e.target.value)}><option>Current month</option><option>Last month</option><option>Custom range</option></select><button className="primary compact" onClick={()=>setNotice('Payment workflow: cash, card, UPI, bank transfer or other.')}><CreditCard size={16}/> Record payment</button></div></div>{notice&&<div className="booking-toast">{notice}<button onClick={()=>setNotice('')}>×</button></div>}<div className="stats"><Stat label="Total income" value="₹4.82L" delta={range}/><Stat label="Expenditure" value="₹1.43L" delta="Includes procurement"/><Stat label="Net profit" value="₹3.39L" delta="Income − expenditure"/><Stat label="Discounts given" value="₹28.4K" delta="Not a cash expense"/><Stat label="Outstanding" value="₹49K" delta="Pending payments"/></div><div className="grid2"><div className="panel chart-panel"><div className="panel-head"><div><b>Income vs expenditure</b><span>Monthly transaction totals</span></div></div><div className="big-chart"><div className="bars">{[35,48,42,58,68,84].map((h,i)=><i key={i} style={{height:`${h}%`}}/>)}</div></div></div><div className="panel"><div className="panel-head"><div><b>Income & spend mix</b><span>Current date selection</span></div></div><div className="pipeline"><Pipeline n="₹3.6L" l="Treatment payments"/><Pipeline n="₹76K" l="Consultation, imaging & lab"/><Pipeline n="₹71K" l="Inventory procurement"/><Pipeline n="₹42K" l="Salary & consultants"/></div></div></div><div className="panel table-panel"><div className="table-head"><b>Transactions</b><button onClick={()=>setNotice('Filter transactions by date, type, category, status or patient.')}>Filter & search</button></div><table><thead><tr><th>Date</th><th>Type</th><th>Category</th><th>Amount</th><th>Method</th><th>Status</th></tr></thead><tbody>{[['Aug 14','Income','Treatment payment','₹12,500','UPI'],['Aug 13','Expense','Inventory / procurement','₹18,600','Bank transfer'],['Aug 12','Expense','Salary','₹24,000','Bank transfer']].map(row=><tr key={row[0]+row[2]}>{row.map((value,index)=><td key={index}>{index===1?<em className="pill">{value}</em>:value}</td>)}<td><em className="pill">Recorded</em></td></tr>)}</tbody></table></div><div className="panel table-panel"><div className="table-head"><div><b>Treatment & Price List</b><span>Master price applies only to new treatment-plan items.</span></div><button onClick={()=>setNotice('Price history is retained; existing plan estimates stay unchanged.')}>Price history</button></div><table><thead><tr><th>Treatment</th><th>Standard price</th><th>Action</th></tr></thead><tbody>{prices.map((t,index)=><tr key={t.name}><td><b>{t.name}</b></td><td><input aria-label={`${t.name} price`} value={t.price} onChange={e=>setPrices(prices.map((price,i)=>i===index?{...price,price:e.target.value}:price))}/></td><td><button className="secondary compact" onClick={()=>setNotice(`${t.name} saved. Historic treatment-plan snapshots are unchanged.`)}>Save</button></td></tr>)}</tbody></table></div></section>}
