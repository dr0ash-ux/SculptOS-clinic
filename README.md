...
Printed prescriptions now use compact two-line medicine entries and current patient sex/contact details, with age calculated at the prescription date. An editable printed meal-timing field is saved with each medicine; longer clinical instructions stay in the record. Existing prescriptions use a concise meal phrase derived from their saved instructions. Clinic name, logo, address and phone come from Admin → Clinic setup. Prescribed dose and non-oral route remain visible.
